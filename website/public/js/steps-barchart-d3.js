window.qd.plotStepsEventsHistory = function() {
    setTimeout(function() {
        $('#steps-graph').empty();
        var data = window.qd.stepsEvents;
        var margin = {
            top: 20,
            right: 30,
            bottom: 30,
        ...
        var svg = d3.select('#steps-graph').append('svg')
            .attr('class', 'chart')
            .attr('width', width)
            .attr('height', height)
            .append('g')
        ...
        var tipText = function (d) {
            return "<strong>" + d.value + (d.value === 1 ? " steps" : " steps") +
