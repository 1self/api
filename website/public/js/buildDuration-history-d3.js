window.qd.plotBuildDurationHistory = function() {
    setTimeout(function() {
        var s = $('#buildDuration-history').empty();
        s = d3.select('#buildDuration-history');

        var w = $("#buildDuration-history").width() * 1;
        var h = w / 1.61;
        ...
        var xOriginForSvg = p[3] + 8;
        var svg = d3.select("#buildDuration-history").append("svg:svg")
            .attr("width", w)
            .attr("height", h)
            .append("svg:g")
            .attr("transform", "translate(" + xOriginForSvg + "," + (h - p[2]) + ")");
        var tip = d3.tip()
            .attr('class', 'd3-tip')
            .offset([-10, 0])
            .html(function(d) {
        ...
        var buildDurationsByResult = d3.layout.stack()(["avgBuildDuration"].map(function(cause) {
            return buildDurationHistory.map(function(d) {
                return {
                    x: parse(d.date),
                    y: +d[cause]
                };
        ...
        x.domain(buildDurationsByResult[0].map(function(d) {
            return d.x;
        }));
        xLinear.domain([0, buildDurationsByResult[0].length]);
        y.domain([0, d3.max(buildDurationsByResult[buildDurationsByResult.length - 1], function(d) {
            return d.y0 + d.y;
        })]);

        // Add a group for each cause.
        var cause = svg.selectAll("g.cause")
            .data(buildDurationsByResult)
            .enter().append("svg:g")
            .attr("class", "cause")
            ...
            .on("click", function(d) {
                if ($(window).width() < 768) {
                    div.transition()
                        .duration(200)
                        .style("opacity", .9);
                    div.html("<strong>" + d.y + " secs</strong> <span style='color:lightgrey'> on " + moment(d.x).format("ddd MMM DD") + "</span>")
