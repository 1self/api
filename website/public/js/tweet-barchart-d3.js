window.qd.plotTweetEventsHistory = function() {
    setTimeout(function() {
        $('#tweet-graph').empty();
        var data = window.qd.tweetEvents;
        var margin = {
            top: 20,
            right: 30,
            bottom: 30,
        ...
        var svg = d3.select('#tweet-graph').append('svg')
            .attr('class', 'chart')
            .attr('width', width)
            .attr('height', height)
            .append('g')
        ...
        var tipText = function (d) {
            return "<strong>" + d.value + (d.value === 1 ? " tweets" : " tweets") +
