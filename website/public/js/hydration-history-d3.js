window.qd.plotHydrationHistory = function () {
    setTimeout(function () {
        $('#hydration-history').empty();
        var data = window.qd.hydrationEvents;
        var margin = {
            top: 20,
            right: 30,
            bottom: 30,
        ...
        var svg = d3.select("#hydration-history").append('svg')
            .attr('class', 'chart')
            .attr('width', width)
            .attr('height', height)
            .append('g')
        ...
        var tipText = function (d) {
            return "<strong>" + d.hydrationCount + (d.hydrationCount === 1 ? " glass" : " glasses") +
        ...
        svg.selectAll('.chart')
            ...
            .attr('y', function (d) {
                return height - margin.top - margin.bottom - (height - margin.top - margin.bottom - y(d.hydrationCount))
            })
            .attr('width', 10)
            .attr('height', function (d) {
                return height - margin.top - margin.bottom - y(d.hydrationCount)
            })
            .on("click", function (d) {
                if ($(window).width() < 768) {
